...
Additionally, the image is converted to greyscale and a discrete Laplacian filter is applied to detect edges and their intensities. Denote the scalar Laplacian at block coordinate $(i, j)$ as $\mathcal{E}_{ij}$
...
*A sample of various pentomino shapes*

From this, the model assigns palette colours to shapes. So, each colour is only available in one particular shape. The model then enumerates all feasible placements of each shape subject to rotation and scaling. A given placement $p$ corresponds to a tile of a specified colour $c_p$ and scale factor $s_p$ placed at an anchor point $(i, j)$. Let $\mathcal{P}$ be the set of all feasible tile placements.

![](./assets/polyominoes.png)
*All feasible placements of an L-tromino on a $2\times 4$ board*
...
$$
\min\sum_{p\in \mathcal{P}}C_px_p
$$
...
### RGBError
The $\mathrm{RGBError}$ component for a placement is simply the squared $L_2$ distance in RGB space between each block in the tile's anchored footprint and the tile colour $c_p$. If $\mathcal{F}_p$ is the block footprint of placement $p$,
$$
\mathrm{RGBError}_p = \sum_{(i, j)\in\mathcal{F}_p}||c_p - B_{ij}||_2^2
$$
...
$$
\mathrm{EdgePenalty}_p = \left(\max_{(i, j)\in \mathcal{F}_p}\mathcal{E}_{ij}\right) \cdot (s_p-1)^2
$$
...
Finally, constraints are added to ensure that all the blocks in the image are covered with no overlaps. Let $\mathcal{A}_{ij}$ be the set of all placements whose footprints contain block $(i, j)$.
$$
\sum_{p\in\mathcal{A}_{ij}}x_{p} = 1, \qquad \forall (i, j)\in \mathcal{R}\times \mathcal{C}
$$